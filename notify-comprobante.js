export default async function handler(req, res) {
  ...
  // Admin panel URL — includes all context so Alina can approve without extra lookups
  ...
  // Fetch comprobante and attach it to the email
  let attachments = [];
  if (fileUrl) {
    try {
...
<div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(120,95,179,.12)">
  ...
  <div style="padding:32px">
    ...
    ${fileUrl ? '<p style="font-size:13px;color:#8a8fa8;margin:0 0 8px">El comprobante está adjunto a este mail. También podés verlo acá:</p><a href="' + fileUrl + '" style="display:inline-block;background:#38464f;color:#fff;text-decoration:none;padding:10px 22px;border-radius:8px;font-size:14px;font-weight:500;margin-bottom:20px">Ver comprobante →</a><br>' : ''}
