export default async function handler(req, res) {
  ...
  // Admin panel URL - incluye todo el contexto para que Alina pueda aprobar sin buscar nada mas
  const adminUrl = `https://www.youlearnba.com/admin-aprobar?email=${encodeURIComponent(studentEmail)}&nombre=${encodeURIComponent(studentName)}&tipo=${encodeURIComponent(classType || '')}&file=${encodeURIComponent(fileUrl || '')}`;

  // Fetch comprobante y adjuntarlo al mail
  let attachments = [];
    if (fileUrl) {
          try {
    ...
    <div style="background:#785fb3;padding:28px 32px;text-align:center">
        <img src="https://www.youlearnba.com/logo.png" alt="YouLearn" style="height:32px">
            ...
            <div style="padding:32px">
                <h2 style="margin:0 0 6px;font-size:18px;color:#785fb3">Nuevo comprobante de pago</h2>
                    <p style="margin:0 0 24px;font-size:14px;color:#8a8fa8">Una alumna subio su comprobante y esta esperando confirmacion.</p>
                                                                                              ...
                                                                                              <td style="padding:10px 0;font-size:13px;color:#8a8fa8">Tipo</td>
                                                                                                      <td style="padding:10px 0;font-size:14px;color:#38464f">
                                                                                                                <span style="background:${classType === 'Individual' ? '#deff83' : '#e5cdf9'};color:${classType === 'Individual' ? '#2d3a1e' : '#785fb3'};padding:3px 10px;border-radius:100px;font-size:12px;font-weight:600">${classType}</span>
                                                                                                                                      ...
                                                                                                                                      ${fileUrl ? '<p style="font-size:13px;color:#8a8fa8;margin:0 0 8px">El comprobante esta adjunto a este mail. Tambien podes verlo aca:</p><a href="' + fileUrl + '" style="display:inline-block;background:#38464f;color:#fff;text-decoration:none;padding:10px 22px;border-radius:8px;font-size:14px;font-weight:500;margin-bottom:20px">Ver comprobante</a><br>' : ''}
                                                                                                                                      
                                                                                                                                          <a href="${adminUrl}" style="display:inline-block;background:#785fb3;color:#fff;text-decoration:none;padding:13px 28px;border-radius:100px;font-size:15px;font-weight:700;margin-top:4px">
                                                                                                                                                Aprobar pack
                                                                                                                                                    </a>
                                                                                                                                                    
                                                                                                                                                        <p style="margin:24px 0 0;font-size:12px;color:#c5bde0">
                                                                                                                                                              Este mail fue generado automaticamente por el portal de YouLearn.
                                                                                                                                                                  </p>
                                                                                                                                                                    </div>
                                                                                                                                                                    </div>
                                                                                                                                                                    </body>
